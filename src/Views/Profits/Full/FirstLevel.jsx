...
const FirstLevel = ({
	...
	useEffect(() => {
		if (referralsTierOne.length > 0) {
			setSelectReferral(referralsTierOne[0]);
			//Traer los paquetes del primer referido por defecto
			getPackagesApi(referralsTierOne[0].value);
		}
	}, [referralsTierOne]);
	...
	const generateExcel = () => {
		TableToExcel.convert(document.getElementById("dataTableReferrals_id"), {
			name: "Information Referrals Tier One.xlsx",
			sheet: {
				name: "Page 1",
			},
		});
	...
	return (
		<section>
			...
			{/* EXPORT IN EXCEL */}
			<ExportExcel
				children={"Download Packages Tier One Referral"}
			...
			<div className="container_selectTable_first_level display_spaceEvenly_center_flex">
				...
				<div className="title_first_referrals">
					<div className="display_right_column">
						...
						<h2 className="text_down_rightTi_payProfits">Tier one</h2>
					</div>
					<div className="select_referrals_tier1 display_column_center_top">
						<Selects
							className={"select_first_referrals"}
							options={referralsTierOne}
